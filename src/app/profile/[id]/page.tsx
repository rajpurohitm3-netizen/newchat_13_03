...
import { generateAESKey, encryptWithAES, encryptAESKeyForUser, importPublicKey } from "@/lib/crypto";
...
function formatLoginTime(date: string | null): string {
  ...
  if (diffHours < 24 && d.getDate() === now.getDate()) return `Today at ${timeStr}`;
...
function getDeviceIcon(device: string | null) {
  ...
  if (lower.includes('iphone') || lower.includes('mobile') || lower.includes('android')) return Smartphone;
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const presenceChannel = supabase.channel("online-users").on("presence", { event: "sync" }, () => {
      ...
      let profileIsOnline = false;
      Object.values(state).forEach((users: any) => {
        users.forEach((u: any) => {
          online.add(u.user_id);
          if (u.user_id === params.id) profileIsOnline = true;
        });
      });
    ...
    // Track current user's login info
  ...
  async function fetchPartner() {
    ...
    if (friendships && friendships.length > 0) {
      ...
      const partner = friendship.sender_id === params.id ? friendship.receiver : friendship.sender;
  ...
  const updateVibe = async () => {
    ...
    try {
      ...
      setProfile({ ...profile, vibe: newVibe });
      setIsEditingVibe(false);
      toast.success("Vibe synchronized");
    } catch (error) {
  ...
  const nudgePartner = async () => {
    ...
    try {
      ...
      const { data: partnerProfile } = await supabase.from("profiles").select("public_key").eq("id", profile.id).single();
      const { data: myProfile } = await supabase.from("profiles").select("public_key").eq("id", currentUserId).single();

      if (partnerProfile?.public_key && myProfile?.public_key) {
        const aesKey = await generateAESKey();
        const encrypted = await encryptWithAES(nudgeText, aesKey);
        const partnerKey = await importPublicKey(partnerProfile.public_key);
        const myKey = await importPublicKey(myProfile.public_key);
        const encryptedKeyForPartner = await encryptAESKeyForUser(aesKey, partnerKey);
        const encryptedKeyForMe = await encryptAESKeyForUser(aesKey, myKey);
        ...
        const packet = JSON.stringify({
          ...
          keys: {
            ...
            [profile.id]: encryptedKeyForPartner
          }
        });
        ...
        if (error) throw error;
        toast.success(`Nudged ${profile.username}!`);
      }
    } catch (error) {
      console.error(error);
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 bg-white/[0.03] border border-white/5 px-4 py-2 rounded-2xl group">
                <Activity className="w-4 h-4 text-emerald-400 group-hover:rotate-12 transition-transform" />
                {isEditingVibe && isOwnProfile ? (
                  <div className="flex items-center gap-2">
                    <input 
                      value={newVibe}
                      onChange={(e) => setNewVibe(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && updateVibe()}
                      autoFocus
                      className="bg-transparent border-none outline-none text-[10px] font-black uppercase tracking-widest text-white w-24"
                    />
                    <button onClick={updateVibe} className="text-emerald-400 hover:text-emerald-300">
                      <Check className="w-3 h-3" />
                    </button>
                  </div>
                ) : (
                  <p 
                    onClick={() => isOwnProfile && setIsEditingVibe(true)}
                    className={`text-[10px] font-black uppercase tracking-[0.2em] ${isOwnProfile ? 'cursor-pointer hover:text-white transition-colors' : ''}`}
                  >
                    Vibe: <span className="text-white">{profile.vibe || "Stable"}</span>
                  </p>
                )}
              </div>
              {!isOwnProfile && (
                <button 
                  ...
                  disabled={isNudging}
                  className="flex items-center gap-2 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 px-4 py-2 rounded-2xl transition-all active:scale-95 disabled:opacity-50"
                >
                  <Hand className={`w-3 h-3 text-indigo-400 ${isNudging ? 'animate-bounce' : ''}`} />
                  <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Nudge</span>
                </button>
              )}
            </div>
          </div>
...
{/* User Stats/Details */}
              ...
              >
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="flex items-center gap-5">
                          ...
                          <div>
                              <p className="text-[9px] font-black uppercase tracking-widest text-white/30">Last Login</p>
                              <p className="text-xs font-black uppercase text-white mt-1">{formatLoginTime(profile.last_login_at)}</p>
                          </div>
                      </div>
                      {profile.last_login_device && (
                        <div className="flex items-center gap-5">
                          {(() => {
                            const DeviceIcon = getDeviceIcon(profile.last_login_device);
                            return (
                          ...
                          <div className="flex-1 min-w-0">
                              <p className="text-[9px] font-black uppercase tracking-widest text-white/30">Active Device</p>
                              <p className="text-[10px] font-black uppercase text-white mt-1 truncate">{profile.last_login_device}</p>
                          </div>
                        </div>
                      )}
                  </div>
          ...
          {partnerProfile && (
            <motion.div
              ...
              className="bg-gradient-to-br from-pink-500/10 via-purple-500/10 to-indigo-500/10 border border-white/10 p-8 rounded-[2.5rem] backdrop-blur-3xl space-y-6"
            >
              <div className="flex items-center justify-between">
                <h2 className="text-xs font-black uppercase tracking-[0.3em] text-white/30 flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-black uppercase text-pink-400/50 bg-pink-400/5 px-3 py-1 rounded-full border border-pink-400/10">Connected</span>
                </div>
              </div>
              
              <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center p-5 bg-white/[0.03] border border-white/5 rounded-2xl group">
                <div className="relative cursor-pointer" onClick={() => router.push(`/profile/${partnerProfile.id}`)}>
                  <motion.div 
                    animate={{ scale: [1, 1.1, 1] }}
                    transition={{ duration: 3, repeat: Infinity }}
                    className="absolute inset-0 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full blur-xl opacity-30" 
                  />
                  <AvatarDisplay profile={partnerProfile} className="h-24 w-24 relative z-10 border-2 border-pink-500/30" />
                  <div className={`absolute bottom-1 right-1 w-5 h-5 rounded-full border-4 border-[#010101] z-20 ${onlineUsers.has(partnerProfile.id) ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]' : 'bg-zinc-700'}`} />
                </div>
                
                <div className="flex-1 min-w-0 w-full">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-2xl font-black uppercase italic truncate text-white hover:text-pink-400 transition-colors cursor-pointer" onClick={() => router.push(`/profile/${partnerProfile.id}`)}>
                        {partnerProfile.full_name || partnerProfile.username}
                      </p>
                      <p className="text-[10px] font-black uppercase tracking-widest text-white/30 mt-0.5">@{partnerProfile.username}</p>
                    </div>
                    <Button 
                      onClick={() => router.push(`/?chat=${partnerProfile.id}`)}
                      size="sm"
                      className="bg-white/5 hover:bg-white/10 border border-white/10 h-10 w-10 p-0 rounded-xl"
                    >
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-xl border border-white/5">
                      ...
                      <span className="text-[9px] font-black uppercase text-white/60">Vibe: {partnerProfile.vibe || "Stable"}</span>
                    </div>
                    {partnerProfile.streak_count > 0 && (
                      <div className="flex items-center gap-1.5 bg-orange-500/10 px-3 py-1.5 rounded-xl border border-orange-500/20">
                        <Flame className="w-3 h-3 text-orange-400" />
                        <span className="text-[9px] font-black text-orange-400">{partnerProfile.streak_count} Days</span>
                      </div>
                    )}
                    <div className="flex items-center gap-1.5 bg-indigo-500/10 px-3 py-1.5 rounded-xl border border-indigo-500/20">
                      <Clock className="w-3 h-3 text-indigo-400" />
                      <span className="text-[9px] font-black text-indigo-400">{formatLoginTime(partnerProfile.last_login_at)}</span>
                    </div>
                  </div>

                  {partnerProfile.last_login_device && (
                    <div className="flex items-center gap-1.5 mt-3 text-white/20">
                      {(() => {
                        const DeviceIcon = getDeviceIcon(partnerProfile.last_login_device);
                        return <DeviceIcon className="w-3 h-3" />;
                      })()}
                      <span className="text-[8px] font-black uppercase tracking-widest truncate">{partnerProfile.last_login_device}</span>
                    </div>
                  )}
                </div>
              </div>

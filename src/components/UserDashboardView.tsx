...
import {
    ...
    Home as HomeIcon, Camera, X, Menu, User, Heart,
...
export function UserDashboardView({ session, privateKey }: UserDashboardViewProps) {
  ...
  const [friendProfiles, setFriendProfiles] = useState<any[]>([]);
    const [advancedSubView, setAdvancedSubView] = useState<"menu" | "vault" | "memories" | "watch" | "countdown">("menu");
  ...
  const unreadNotificationCount = notifications.filter(n => !n.read).length;
    ...
    const navItems = [
      { id: "dashboard", icon: HomeIcon, label: "Chatify" },
      ...
      const advancedFeatures = [
            ...
            { id: "countdown", icon: Clock, label: "Timeline", desc: "Neural Continuity Timer", color: "from-indigo-600 to-blue-600" },
          ];
                  ...
                  {[
                    ...
                    <div className="mb-4">
                      <AnimatePresence mode="wait">
                        {!showCountdown ? (
                          <motion.div
                            key="countdown-trigger"
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.95 }}
                            onClick={() => setShowCountdown(true)}
                            className="p-10 bg-gradient-to-br from-indigo-950/40 to-blue-950/40 border border-indigo-500/20 rounded-[2.5rem] cursor-pointer group relative overflow-hidden shadow-2xl"
                          >
                            <div className="absolute inset-0 bg-indigo-500/5 group-hover:bg-indigo-500/10 transition-colors" />
                            <div className="relative z-10 flex flex-col items-center text-center space-y-4">
                              <div className="p-4 bg-indigo-500/20 rounded-2xl group-hover:scale-110 transition-transform duration-500">
                                <Clock className="w-10 h-10 text-indigo-400" />
                              </div>
                              <div>
                                <h3 className="text-2xl font-black italic uppercase tracking-tighter text-white">Journey <span className="text-indigo-500">Timeline</span></h3>
                                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 mt-2">Tap to visualize neural continuity</p>
                              </div>
                            </div>
                            <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-indigo-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                          </motion.div>
                        ) : (
                          <motion.div
                          ...
                          >
                            <div className="absolute -top-4 -right-4 z-50">
                              <Button 
                                ...
                                className="bg-black/50 backdrop-blur-md border border-white/10 text-white/40 hover:text-white rounded-full h-10 w-10 shadow-2xl"
                              >
                                <X className="w-5 h-5" />
                              </Button>
                            </div>
                  ...
                  <div className="p-6 sm:p-8 pb-32 lg:pb-12">
                          ...
                          <div className="mb-10">
                            <AnimatePresence mode="wait">
                              {!showCountdown ? (
                                <motion.div
                                  key="adv-countdown-trigger"
                                  initial={{ opacity: 0, scale: 0.98 }}
                                  animate={{ opacity: 1, scale: 1 }}
                                  exit={{ opacity: 0, scale: 0.95 }}
                                  onClick={() => setShowCountdown(true)}
                                  className="p-10 bg-zinc-900/50 border border-zinc-800 rounded-[3rem] cursor-pointer group relative overflow-hidden shadow-2xl flex flex-col items-center text-center space-y-4"
                                >
                                  <div className="absolute inset-0 bg-indigo-500/5 group-hover:bg-indigo-500/10 transition-colors" />
                                  <div className="p-4 bg-indigo-500/20 rounded-2xl group-hover:scale-110 transition-transform duration-500">
                                    <Clock className="w-10 h-10 text-indigo-400" />
                                  ...
                                  <div>
                                    <h3 className="text-2xl font-black italic uppercase tracking-tighter text-white">Journey <span className="text-indigo-500">Timeline</span></h3>
                                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/30 mt-2">Activate temporal continuity visualization</p>
                                  </div>
                                  <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-indigo-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                                </motion.div>
                              ) : (
                                <motion.div
                                  key="adv-countdown-active"
                                  initial={{ opacity: 0, scale: 0.95 }}
                                  animate={{ opacity: 1, scale: 1 }}
                                  className="relative"
                                >
                        ...
                        <div className="h-full flex flex-col">
                          <div className="p-6 border-b border-white/5 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Temporal Matrix</span>
                            </div>
                          </div>

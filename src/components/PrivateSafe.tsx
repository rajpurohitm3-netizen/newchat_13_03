...
interface SafeItem {
  ...
  async function deleteItem(id: string, fileUrl: string) {
    ...
    if (isSettingPassword) {
      return (
        <div className="fixed inset-0 z-[110] bg-zinc-950 flex flex-col items-center justify-center p-4 sm:p-6 text-center">
          <div className="max-w-md w-full space-y-6 sm:space-y-8 bg-zinc-900/50 p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-zinc-800 backdrop-blur-3xl relative overflow-hidden">
            <div className="absolute inset-0 bg-indigo-500/5 pointer-events-none" />
            <div className="flex justify-center">
              <motion.div 
                animate={{ scale: [1, 1.1, 1], rotate: [0, 5, -5, 0] }}
                transition={{ duration: 4, repeat: Infinity }}
                className="p-4 sm:p-6 bg-indigo-500/10 rounded-full border border-indigo-500/20"
              >
                <ShieldCheck className="w-12 h-12 sm:w-16 sm:h-16 text-indigo-500" />
              </motion.div>
            </div>
            <div className="space-y-2 relative z-10">
              <h2 className="text-2xl sm:text-3xl font-black uppercase italic tracking-tighter">Initialize Vault</h2>
              <p className="text-zinc-500 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Establish your encrypted media container</p>
            </div>
            <div className="space-y-3 sm:space-y-4 relative z-10">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Input 
                    ...
                    className="h-12 sm:h-14 bg-zinc-800/50 border-zinc-700 rounded-xl sm:rounded-2xl px-4 sm:px-6 focus:border-indigo-500/50 transition-all"
                  />
                  <button 
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-indigo-400 transition-colors"
                  >
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="h-12 sm:h-14 bg-zinc-800/50 border-zinc-700 rounded-xl sm:rounded-2xl px-4 sm:px-6 focus:border-amber-500/30 transition-all"
                />
              </div>
              <Button 
                onClick={async () => {
                ...
                disabled={isLoading}
                className="w-full h-14 sm:h-16 rounded-xl sm:rounded-2xl bg-indigo-600 hover:bg-indigo-500 font-black uppercase tracking-[0.3em] text-[9px] sm:text-[10px] shadow-xl shadow-indigo-900/20 active:scale-[0.98] transition-all"
              >
                {isLoading ? <Loader2 className="w-5 h-5 sm:w-6 sm:h-6 animate-spin" /> : "Establish Node"}
              </Button>
    ...
    if (isLocked) {
      return (
        <div className="fixed inset-0 z-[110] bg-zinc-950 flex flex-col items-center justify-center p-4 sm:p-6 text-center">
          <div className="max-w-md w-full space-y-6 sm:space-y-8 bg-zinc-900/50 p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-zinc-800 backdrop-blur-3xl">
            <div className="flex justify-center">
              <div className="p-4 sm:p-6 bg-zinc-800 rounded-full">
                <Lock className="w-12 h-12 sm:w-16 sm:h-16 text-zinc-600" />
              </div>
            </div>
            <div className="space-y-2">
              <h2 className="text-2xl sm:text-3xl font-bold uppercase tracking-tight">Personal Safe</h2>
              <p className="text-zinc-500 text-[9px] sm:text-sm font-semibold uppercase tracking-widest sm:text-[10px]">Protected Layer</p>
            </div>
            <div className="space-y-3 sm:space-y-4">
              <Input 
                ...
                className="h-12 sm:h-14 bg-zinc-800 border-zinc-700 rounded-xl sm:rounded-2xl px-4 sm:px-6 text-center tracking-[0.5em]"
              />
              <Button onClick={handleUnlock} className="w-full h-12 sm:h-14 rounded-xl sm:rounded-2xl bg-indigo-600 hover:bg-indigo-700 font-black uppercase tracking-widest text-[10px] sm:text-xs shadow-xl shadow-indigo-900/20">Unlock Safe</Button>
              
              <div className="flex flex-col gap-2 pt-4">
                <button 
                  onClick={() => setShowForgotPassword(true)}
                  className="text-[10px] sm:text-xs font-bold text-indigo-400 hover:text-indigo-300 uppercase tracking-widest"
                >
                  Forgot Vault Password?
                </button>
                <Button variant="ghost" onClick={onClose} className="w-full text-zinc-500 font-bold uppercase tracking-widest text-[9px] sm:text-[10px]">Back to Dashboard</Button>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-zinc-950 flex flex-col font-sans">
      <div className="p-4 sm:p-6 border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-3xl flex items-center justify-between pt-10 sm:pt-12">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-black uppercase italic tracking-tighter">Private Safe</h2>
            <p className="text-[9px] sm:text-[10px] text-zinc-500 font-black uppercase tracking-widest">Secured Storage</p>
          </div>
        </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <Button 
              ...
              variant="ghost" 
              size="icon" 
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-indigo-400"
            >
            ...
            <Button 
              ...
              variant="ghost" 
              size="icon" 
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white"
            >
          ...
          <div className="max-w-6xl mx-auto py-4 sm:py-6">
            <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-4">
              <button 
                ...
                className="aspect-square rounded-2xl sm:rounded-3xl border-2 border-dashed border-zinc-800 hover:border-indigo-500 transition-all flex flex-col items-center justify-center gap-2 sm:gap-3 group bg-zinc-900/20"
              >
                {isUploading ? (
                  <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-500 animate-spin" />
                ) : (
                  <>
                    <div className="p-3 sm:p-4 bg-zinc-900 rounded-xl sm:rounded-2xl group-hover:scale-110 transition-transform">
                      <Plus className="w-6 h-6 sm:w-8 sm:h-8 text-zinc-500 group-hover:text-indigo-500" />
                    </div>
                    <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-zinc-600 group-hover:text-indigo-500">Add</span>
                  </>
                )}
              </button>
              ...
              {safeItems.map((item) => (
                <motion.div 
                  ...
                  key={item.id} 
                  className="aspect-square rounded-2xl sm:rounded-3xl bg-zinc-900 border border-zinc-800 overflow-hidden relative group"
                >
                  {item.type === 'photo' && (
                    ...
                    {item.type === 'video' && (
                      <div className="w-full h-full bg-zinc-800 flex items-center justify-center">
                        <VideoIcon className="w-8 h-8 sm:w-10 sm:h-10 text-zinc-600" />
                        <div className="absolute inset-0 bg-black/20" />
                      </div>
                    )}

                  {item.type === 'audio' && (
                    <div className="w-full h-full bg-indigo-900/20 flex flex-col items-center justify-center p-2 sm:p-4">
                      <Music className="w-8 h-8 sm:w-10 sm:h-10 text-indigo-500 mb-1 sm:mb-2" />
                      <p className="text-[7px] sm:text-[8px] font-black text-center truncate w-full text-indigo-300 uppercase px-1 sm:px-2">{item.file_name}</p>
                    </div>
                  )}
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 sm:gap-3">
                    <Button 
                      ...
                      size="icon" 
                      variant="ghost" 
                      className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl bg-white/10 hover:bg-white/20 text-white border border-white/10"
                    >
                    ...
                    <Button 
                      ...
                      size="icon" 
                      variant="ghost" 
                      className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl bg-red-500/20 hover:bg-red-500/40 text-red-500 border border-red-500/20"
                    >
                  ...
                  <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3">
                    {item.type === 'photo' && <ImageIcon className="w-3 h-3 sm:w-4 sm:h-4 text-white drop-shadow-lg" />}
                    {item.type === 'video' && <VideoIcon className="w-3 h-3 sm:w-4 sm:h-4 text-white drop-shadow-lg" />}
                    {item.type === 'audio' && <Music className="w-3 h-3 sm:w-4 sm:h-4 text-white drop-shadow-lg" />}
                  </div>
                </motion.div>
              ))}
            </div>
              ...
              {safeItems.length === 0 && !isLoading && (
                <div className="py-12 sm:py-20 text-center opacity-30 flex flex-col items-center space-y-3 sm:space-y-4">
                  <div className="p-6 sm:p-10 rounded-full border-4 border-dashed border-zinc-800">
                    <ShieldCheck className="w-12 h-12 sm:w-16 sm:h-16" />
                  </div>
                  <div>
                    <p className="text-lg sm:text-xl font-black uppercase italic">Empty</p>
                    <p className="text-[10px] sm:text-xs font-bold uppercase tracking-widest mt-1">Add items using the plus button</p>
                  </div>
                </div>
              )}
            </div>

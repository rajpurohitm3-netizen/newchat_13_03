...
function VoiceMessage({ url, isMe }: { url: string; isMe: boolean }) {
  ...
  return (
    <div className={`p-4 rounded-[2rem] flex items-center gap-3 min-w-[200px] ${isMe ? 'bg-indigo-600' : 'bg-white/5 border border-white/10'}`}>
  ...
  const saveToVault = async (message: any) => {
    ...
    const sendAudioMessage = async () => {
      ...
      if (!error) {
        ...
        toast.error("Failed to upload audio packet");
      }
    };
    ...
    <div className="flex flex-col h-full bg-[#030303] relative overflow-hidden">
                  ...
                  <div className={`max-w-[85%] flex flex-col ${isMe ? "items-end" : "items-start"} group relative`}>
                    {msg.media_type === 'snapshot' ? (
                      ...
                      <video src={msg.media_url} controls className="rounded-[2.5rem] border border-white/10 max-h-80 shadow-2xl" />
                        ...
                        <VoiceMessage url={msg.media_url} isMe={isMe} />
      ...
      <footer className="p-6 bg-black/40 backdrop-blur-3xl border-t border-white/5 shrink-0">
          ...
          <AnimatePresence>
            {(isRecording || recordedAudioUrl) && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="mb-4 bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-[2rem] p-4 shadow-2xl"
              >
                {isRecording ? (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-red-400">
                        {isRecordingPaused ? 'Recording Paused' : 'Recording Signal...'} {Math.floor(recordingDuration / 60)}:{String(recordingDuration % 60).padStart(2, '0')}
                      </span>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" onClick={cancelRecording} className="text-white/20 hover:text-red-400 hover:bg-red-500/10 rounded-xl"><Trash2 className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" onClick={isRecordingPaused ? resumeRecording : pauseRecording} className="text-white/20 hover:text-white hover:bg-white/10 rounded-xl">
                        {isRecordingPaused ? <Play className="w-4 h-4" /> : <Pause className="w-4 h-4" />}
                      </Button>
                      <Button onClick={stopRecording} className="bg-red-500 hover:bg-red-600 rounded-xl px-4 text-[10px] font-black uppercase tracking-widest text-white">Stop</Button>
                    </div>
                  </div>
                ) : recordedAudioUrl ? (
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <VoiceMessage url={recordedAudioUrl} isMe={true} />
                    </div>
                    <div className="flex gap-2 ml-4">
                      <Button variant="ghost" size="icon" onClick={cancelRecording} className="text-white/20 hover:text-red-400 hover:bg-red-500/10 rounded-xl"><X className="w-4 h-4" /></Button>
                      <Button onClick={sendAudioMessage} className="bg-indigo-600 hover:bg-indigo-700 rounded-xl px-4 text-[10px] font-black uppercase tracking-widest text-white flex items-center gap-2">
          ...
          <div className="flex items-center gap-3 relative">
            ...
            {!newMessage.trim() && !recordedAudioUrl ? (
              <Button onClick={isRecording ? stopRecording : startRecording} className={`h-12 w-12 rounded-2xl ${isRecording ? 'bg-red-600 animate-pulse' : 'bg-white/5 text-white/40'} hover:bg-indigo-500 transition-all shadow-lg`}>
                <Mic className="w-5 h-5" />
              </Button>
            ) : (
              <Button onClick={recordedAudioUrl ? sendAudioMessage : () => sendMessage()} className="h-12 w-12 rounded-2xl bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-600/20"><Send className="w-5 h-5" /></Button>
            )}

...
export function VideoCall({ 
  ...
  const createPeerConnection = useCallback((localStream: MediaStream) => {
    ...
    pc.ontrack = (event) => {
      ...
      setRemoteStream(prev => {
        ...
        // Always return a NEW MediaStream object to trigger React re-render
  ...
  useEffect(() => {
    ...
    const startCall = async () => {
      try {
        console.log("Starting call setup, isInitiator:", isInitiator);
        if (contact.public_key) {
          partnerPublicKeyRef.current = await importPublicKey(contact.public_key);
        }
        ...
        const channel = supabase.channel(`call_v3_${channelId}`)
          .on('broadcast', { event: 'signal' }, async ({ payload }) => {
            if (payload.userId === userId) return; // Ignore own signals
            if (!peerConnection.current) return;

            console.log(`Received signal broadcast: ${payload.type}`);
            const signalData = await decryptSignal(payload.signal_data);

            if (payload.type === "ready-to-join" && isInitiator) {
              console.log("Partner ready, creating offer");
              const offer = await peerConnection.current.createOffer();
              await peerConnection.current.setLocalDescription(offer);
              sendSignal("offer", { sdp: peerConnection.current.localDescription });
            } else if (payload.type === "offer" && !isInitiator && !remoteDescriptionSet.current) {
          ...
          .subscribe(async (status) => {
            if (status === 'SUBSCRIBED' && isMounted) {
              ...
              if (isInitiator) {
                ...
                sendSignal("offer-ping", { mode: initialCallType });
              } else {
                setConnectionStatus("Joining...");
                sendSignal("ready-to-join", {});
                
                // If we already have the incoming signal (SDP) from the database
                if (incomingSignal && incomingSignal.sdp && !remoteDescriptionSet.current) {
                   console.log("Using initial incoming signal from DB");
